      ...
      <div className="flex-1 p-6 md:p-12 overflow-y-auto relative">
        ...
        <div className={`max-w-7xl mx-auto transition-all duration-300 ${!isSidebarVisible ? "md:pl-12" : ""}`}>
          <div className="mb-8 border-b border-zinc-800 pb-5">
            <h1 className="text-2xl font-black tracking-tight text-white">Users Management</h1>
            <p className="text-sm text-zinc-400 mt-1">
              Active system profiles directory
            </p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-center border-collapse">
                ...
                <tbody className="divide-y divide-zinc-800/50 text-sm">
                  {users.map((u) => (
                    <tr key={u.id} className="group hover:bg-zinc-900/20 transition-colors">
                      <td className="py-4 font-mono font-medium text-zinc-400">{u.id}</td>
                      <td className="py-4 text-zinc-400 max-w-xs truncate pr-4" title={u.name}>{u.name}</td>
                      <td className="py-4 text-zinc-400 max-w-xs truncate pr-4" title={u.email_id}>{u.email_id}</td>
                      ...
                      <td className="py-4">
                        <div className="flex justify-center items-center gap-2">
                          <button 
                            ...
                            className="bg-red-500/20 hover:bg-red-500 text-red-400 hover:text-white p-1.5 rounded-md transition"
                          >
                            Delete
                          </button>
                        </div>
